...
public class DAOncc {
	...
	public boolean openConnection() {
		try {
			...
			String connectionUrl = "jdbc:sqlserver://LAPTOP-4DA46HIV:1433;DatabaseName=QuanLyNhaThuoc;encrypt=true;trustServerCertificate=true;";
	...
	public boolean addNCC(NhaCungCap ncc) { 
		...
		if (openConnection()) {
			try {
				String sql = "Insert into tblNCC values(?,?,?,?)";
	...
	public boolean checkMaNCC(String maNCC){
		...
		if (openConnection()) {
			try { 
				...
				String sql = "Select * from tblNCC where MaNCC = ?";
				PreparedStatement stmt = con.prepareStatement(sql);
	...
	public boolean checkPhoneNumber(String phoneNumber){
			...
			if (openConnection()) {
				try { 
					...
					String sql = "Select * from tblNCC where PhoneNumber="+phoneNumber;
					Statement stmt = con.createStatement();
					ResultSet rs = stmt.executeQuery(sql);
					result = rs.next();
	...
	public boolean updateNCC(NhaCungCap ncc) {
	    ...
	    if (openConnection()) {
	        try {
	            String sql = "UPDATE tblNCC SET TenNCC = ?, PhoneNumber = ?, Address = ? WHERE MaNCC = ?";
	            PreparedStatement stmt = con.prepareStatement(sql);
	...
	public ArrayList<NhaCungCap> findNCCByNameOrId(String searchKeyword) {
        ...
        if (openConnection()) {
            try {
                String sql = "SELECT MaNCC, TenNCC ,PhoneNumber, Address FROM tblNCC WHERE MaNCC = ? OR TenNCC LIKE ?";
                PreparedStatement stmt = con.prepareStatement(sql);
	...
	public String getTenNCC(String maNCC) {
		...
		if (openConnection()) {
			...
			try {
				String sql = "SELECT TenNCC FROM tblNCC WHERE MaNCC = ?";
				PreparedStatement stmt = con.prepareStatement(sql);
				stmt.setString(1, maNCC);
				ResultSet rs = stmt.executeQuery();
				if (rs.next()) {
